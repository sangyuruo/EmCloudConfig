...
WORK_DIR=/opt/cloud
serviceJar="$WORK_DIR/EmCloudRegister.war"
serviceNameLo="emcloudregister"                                  # service name with the first letter in lowercase
serviceName="EmCloudRegister"                                    # service name
serviceUser="root"                                         # OS user name for the service
serviceGroup="root"                                        # OS group name for the service
applDir="/opt/cloud"				       # home directory of the service application

SCRIPT_DIR=$(cd "$(dirname "$0")"; pwd)
# Set this to your Java installation
serviceUserHome="$SCRIPT_DIR/$serviceUser"                 # home directory of the service user
...
javaArgs="-jar $serviceJar"                                # arguments for Java launcher

javaCommandLine="$javaExe $javaArgs"                       # command line to start the Java service application
javaCommandLineKeyword="EmCloudOU"                         # a keyword that occurs on the commandline, used to detect an already running service process and to distinguish it from others
